...
export class HeuristicsPlayerAI extends RandomPlayerAI {
	...
	protected _should_switch_out(request) {
	    ...
	    // Perish Song: switch only if active mon is under perish and NOT trapped
	    const activeSelf = request.side.pokemon.find(mon => mon.isActive === true);
	    if (
		activeSelf &&
		Object.keys(activeSelf.volatiles || {}).some(v => v.includes("perish")) &&
		!activeSelf.trapped
	    ) {
		return true;
	    }
