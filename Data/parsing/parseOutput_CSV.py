...
def save_win_matrix_to_csv(battle_matrix, filename):
    # Get list of all trainers
    trainers = sorted(battle_matrix.keys())
    with open(filename, mode='w', newline='') as file:
        ...
        for trainer1 in trainers:
            ...
            for trainer2 in trainers:
                if trainer1 == trainer2:
                    row.append(0)  # self-matchups are 0
                else:
                    row.append(battle_matrix[trainer1][trainer2]['wins'])
            writer.writerow(row)
...
if __name__ == "__main__":
    ...
    file_path = 'Tour100/output1.txt'  # Hardcoded to parse only output1.txt
